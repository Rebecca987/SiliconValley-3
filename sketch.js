...
function preload(){
   ...
   // infoButton=loadImage("InfoButton.jpg");
    ...
    treasureImg=loadImage("Animals/treasure.gif")
...
createCanvas(windowWidth,windowHeight)
   ...
   // bglevel1.visible=false
    ...
    treasure = createSprite(600,400,100,100);
...
function draw(){
   ...
   if (keyDown("left")) {
    ...
    if(gamestate==="story"){
        ...
        text("After falling out of your ship, you have woken up at the bottom of the sea.", 200,150)
        text("You see new species of fish so you decide to collect them", 300,200)
        text("but you know that the sharks down there have a dangerous bite", 260,250)
        text("so you need to avoid them and the bombs are also falling into the sea!", 220,300)

        textSize(30)
        fill("white")
        text("Avoid Sharks and Bombs!",550,550)
...
function fishes1(){
  ...
  function crabs(){
    if(World.frameCount%79 === 0){
      crab = createSprite(850,120,100,100);
      crab.scale = 0.3;
